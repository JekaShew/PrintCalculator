...
import { faBan, faTimes, faPenToSquare, faCheck } from '@fortawesome/free-solid-svg-icons';
import './form.css';
...
class Form extends Component {
    ...
    renderProp(prop, key, crud) {
        ...
        if (crud == true) {
            switch (prop.type)
            {
                ...
                case propType.table:
                    ...
                    console.log(this.props.data[prop.name]);
                    return (
                        <InputTable
                            ui={prop.ui || this.props.ui}
                            key={key}
        ...
        {
            console.log("renderPropElse");
            return
            (
                <div className="label" key={key} >
                    { this.props.data[prop.name] }
                </div>              
            );
        }
    }
        ...
        if (!crud) {
            ...
            return(
                <Button
                    ...
                    ui={ui.d.secondary}
                    icon={faPenToSquare}
                    onClick={() => this.props.onEditBtnClick(this.porps.data.id)}
                />
            )
        }
    }
    ...
    renderProps() {
        ...
        if (this.props.model.rows)
                                        ...
                                        <div className={"props-col " + col.width}>
                                            {
                                                ...
                                                col.props.map((prop, k) => this.renderProp(prop, i * 50 + j * 5 + k, this.props.crud))
            ...
            return (
                <div className="props">
                    {
                        this.props.model.props.map((prop, i) => this.renderProp(prop, i, this.props.crud))
                    }
                </div>
            );
    }
    ...
    render() {
        return (
            <div className={"form " + (this.props.ui || 'x1')}>
                {this.renderProps()}
                {this.renderChildren()}
                {this.renderControls(this.props.crud)}
            </div>
        );
    }
}
